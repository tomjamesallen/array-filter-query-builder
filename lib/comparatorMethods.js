...
exports.default = ComparatorMethods;
...
function ComparatorMethods(config) {
  function convertNumberToString(val) {
    if (config.coerceStringCompatators && typeof val === 'number') {
      val = val.toString();
    }
    return val;
  }
  ...
  function convertStringToNumber(val) {
    if (config.coserceNumericComparators && typeof val === 'string') {
      val = parseFloat(val);
    }
    return val;
  }
